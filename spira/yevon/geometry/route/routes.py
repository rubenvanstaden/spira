...
__all__ = ['Route', 'RouteStraight', 'RoutePath']
...
class Route(spira.Polygon):
    ...
    def create_ports(self, ports):
        ...
        p1 = spira.Port(name='P1',
            midpoint=self.shape.m1,
            width=self.shape.w1,
            orientation=self.shape.o1,
        )

        p2 = spira.Port(name='P2',
            midpoint=self.shape.m2,
            width=self.shape.w2,
            orientation=self.shape.o2,
        )
...
def RoutePath(path, width, layer):

    shape = RoutePointShape(path=path, width=width)
    R = Route(shape=shape, layer=layer)
    # T = vector_match_transform(v1=R.ports[0], v2=p1)
    # R.transform(T)
    return R
