...
ply = spira.Polygons(shape=shape, gds_layer=spira.Layer(number=88))
ply.move(midpoint=(1*1e6, 1*1e6), destination=(0*1e6, 0*1e6))
ply2 = spira.Polygons(shape=deepcopy(shape), gds_layer=spira.Layer(number=88))
...
ply3 = spira.Polygons(shape=shape1, gds_layer=spira.Layer(number=100))
