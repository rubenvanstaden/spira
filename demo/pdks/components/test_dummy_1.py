...
class TIntersection(Circuit):
    ...
    def create_ports(self, ports):
        ...
        ports += spira.Term(name='T0', midpoint=[60*1e6, 10*1e6], orientation=90)
        ports += spira.Dummy(name='D0', midpoint=[50*1e6, 10*1e6], orientation=-90)

        for i in range(self.num):
            ports += spira.Term(name='T{}'.format(i+3), midpoint=[(i+1)*self.pos, self.dy], orientation=180)
            ports += spira.Dummy(name='D{}'.format(i+1), midpoint=[(i+1)*self.pos, 0])
