...
from core.descriptor import DataField
from core.descriptor import FunctionField
from core.descriptor import DataFieldDescriptor


def CoordField(**kwargs):
    from spira.geometry.coord import Coord
...
def TransformationField(name='noname', number=0, datatype=0, **kwargs):
    from core.tranformation import Transform
    # if 'default' not in kwargs:
...
def PortField(midpoint=[0, 0], **kwargs):
    # from spira.gdsii.port import Port
    from spira.geometry.ports.port import Port
    if 'default' not in kwargs:
...
def TermField(midpoint=[0, 0], **kwargs):
    from spira.gdsii.term import Term
    if 'default' not in kwargs:
...
def ShapeField(points=[], doc='', **kwargs):
    from spira.geometry.shapes.shape import Shape
    if 'default' not in kwargs:
